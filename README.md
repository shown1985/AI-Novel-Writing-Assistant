...
- 单书工作台会分别展示已保存正文、本轮任务范围和全书目标，并在桌面与移动端统一提示任务状态、局部质量项和一个可信的建议下一步；局部任务完成不会被误显示为整本书已完成，读取异常也不会清空已保存成果。
- 打开、刷新或重新聚焦设置、模型路由和知识库状态页时只读取最近一次连接结果，不会自动向模型、embedding 服务或向量库发起检测；真正的连接检查只由明确的检测操作触发，连续点击也不会重复执行同一次检测。
- 设置和知识库会分别说明尚未检测、配置已变化、检测失败、状态读取失败和检测中；基础模型配置有效时，尚未检测或旧结果过期不会阻止开始创作，知识库未检测也不会显示为连接失败。
- 模型连接检测会保存最近结果和兼容建议，应用重启后仍可追溯；配置发生变化时旧结果会标记为过期，本次失败也不会覆盖之前的检测记录。
