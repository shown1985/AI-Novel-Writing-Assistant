...
- Apple Silicon Mac 可以在本机生成并运行桌面客户端，提供应用程序、DMG 安装盘和 ZIP 包；Windows 安装版继续使用原有构建方式。
- Mac 桌面版会携带本地写作服务、页面资源、数据库能力和图片处理能力，并检查安装包中的关键组件是否属于 arm64。
- 关闭 Mac 客户端的最后一个窗口后，可以从 Dock 再次打开工作区，本地写作服务会继续复用。
- 本地测试包不会连接公开更新通道；后续完成 Apple 签名与公证后再启用公开更新。
