...
- 世界骨架生成遇到地点或故事入口输出过长、不完整时，会自动拆成更小批次继续，并保留已经完成的内容。
- 正式保存前即使重启应用，已完成的世界生成也会直接回到骨架预览，无需重新填写或重新生成。
- 视觉资源库会兼容旧版桌面数据；单个素材来源异常时仍可浏览其他来源，加载问题会在资源库内集中显示，不再连续弹出多条错误提示。
- 写法引擎会清楚区分个人写法与官方起步写法，并用左侧选择、右侧分区阅读的方式展示长内容；应用、试写和去 AI 味入口更集中，窗口较窄时也不会把中文挤成逐字换行。
- 在世界样本手册中编辑概要、规则、势力、地点或张力时，会进入专注的编辑视图，可保存后返回手册，或取消本次改动后返回。
