...
- 保存世界字段和核心公理时会依据当前内容版本安全提交；网络重试会沿用同一次保存意图，减少重复写入和误覆盖。
- 候选验收中：AI 实况窗口可区分本次调用的预计模型、实际采用模型和备用链，帮助作者理解重试后的结果；该能力仍等待用户验收，不代表正式发布完成。

#### 修复

- 修复世界保存请求在版本过期、重复提交或缺少必要保护信息时可能静默覆盖较新内容的问题；系统会明确提示冲突或需要重新读取，已保存内容不会被错误请求覆盖。
