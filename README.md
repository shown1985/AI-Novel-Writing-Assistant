...
- 单书工作台的建议下一步可以直接在当前创作页面执行；桌面与移动端会先说明影响范围和已保存成果保护，提交期间防止重复操作，并在原位置显示提交结果或失败后的重试指引。

#### 优化

- 执行建议后会重新读取这本书的正式任务状态；请求被后台接收但后续执行失败时，页面不会误报完成，而会保留已有成果并给出当前可用的恢复入口。
- 执行详情与运行记录继续用于查看进度、产物、错误和来源位置；需要继续或恢复时会引导作者回到对应创作页面操作，避免同一任务出现多个相互冲突的主操作。
