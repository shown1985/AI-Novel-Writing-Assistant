...
- 简易书架会记住每部作品最近阅读的章节和位置；刷新、重新进入或使用浏览器前进后退时可以继续阅读，切换作品不会带入上一部作品的阅读现场。
- 专业章节编辑器以正文为中心，章节参考与 AI 协作可按需展开；宽屏可以同时使用两个辅助区域，窄屏一次只显示一个，折叠和重新打开不会丢失当前编辑现场。
- 章节 AI 诊断只在作者打开辅助区域或明确发起修订时运行；仅进入页面、折叠重开或选择正文不会重复调用。遇到外部正文变化时会先请作者选择，避免静默覆盖草稿。

#### 修复

- 切换作品后，上一部作品迟到的继续、恢复、审批、重试或取消结果不会改写当前作品现场；需要人工恢复的暂停也不会被后台轮询清除。
- AI 资料检索与写法提取的数值配置在未填写、留空或包含坏值时会采用可靠默认值，同时保留作者明确设置的合法低值与允许的零值。
...
- 修复在一个世界中处理一致性问题时可能先改写另一个世界记录的问题；错归属或不存在的请求不会改变任何问题状态。
...
Release 1 不开放局域网或公网访问。前端开发服务器与业务 API 都只监听本机回环地址；需要其他设备访问时，应等待带认证、MFA 和工作区授权的 Release 2，不能仅通过修改 CORS 或把服务绑定到 `0.0.0.0` 暴露作品和模型密钥。

只有在这些场景下，才建议创建 `client/.env`：

- 你需要在同一台机器上固定一个回环 API 地址
- 你正在排查本机代理配置

通常建议删除或注释 `VITE_API_BASE_URL`，让开发服务器使用同源代理。
...
- 以本地 SQLite 和本机回环访问为基础，无需账号
- 优先完成自动导演、连续写章、恢复、世界与角色维护、导出和桌面发布闭环
- 不开放多人协作、LAN登录或云端作品自动同步

### Release 2：MySQL 与多人协作版

- 以中心 MySQL 承载协作作品，个人本地作品继续保留
- 引入首位管理员、MFA、分权账号、作品成员、评论、修改建议和实时共编
- 本地作品只有在用户明确选择后才迁入协作空间

Release 1/2 是产品里程碑，不替代日期更新记录和桌面安装包版本。
