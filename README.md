...
- 手机端从底部“任务”入口进入“运行记录”，“更多”菜单只列出其余页面，入口不重复。
- 手机端打开“热门题材雷达”，以及“模型与厂商”“自动导演设置”“知识库与写法”“桌面与维护”这几个设置子页面时，内容完整适配屏幕宽度，无需左右滑动。
- 手机端“运行记录”页的筛选区排列更紧凑：类型、状态、异常筛选集中在第一行，关键词搜索和排序方式集中在第二行，两行即可完成筛选。
- 参考拆书写法生成正文时，写法要求中出现的原作人名、地名、书名会在交给 AI 之前自动隐去，减少正文照搬原作专有名词的情况。
- 拆书分析批量生成角色档案时，模型用量统计更准确，用量上限提醒能按实际消耗及时生效。
- 接管已有项目时选择写到第几章，选择生产方式开始正文后会在该章停下，不会继续写完整本书。
