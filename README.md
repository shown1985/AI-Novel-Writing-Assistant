...
- 修复本书世界内容变化后，规划、角色或章节生成可能继续沿用旧世界切片的问题；刷新和生成只采用与当前本书世界匹配的结果，旧作品已有的有效切片仍可继续使用。
- 修复世界手册和高级结构编辑保存时可能覆盖较新设定的问题；版本冲突会保留当前草稿，结果不确定时可按原保存意图重试，历史快照失败也不会被误报为世界内容保存失败。
