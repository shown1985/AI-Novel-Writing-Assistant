...
- 使用 GLM-5.3 Flash 等推理模型生成世界骨架时，会优先采用较低的思考深度，让更多输出额度用于完整的世界设定。
- 世界骨架没有生成正文时，向导会区分思考额度耗尽、输出达到上限和服务无响应，并给出下一步处理建议。
- 生成失败不会保存不完整的世界数据，JSON 修复会沿用本次生成的思考深度。
- 标准长篇和复杂群像会分阶段整理世界规则、势力、地点和关系，再统一检查后生成开局入口，降低大规模世界生成中断的概率。
- 世界骨架生成会保留阶段进度；遇到中断时可从世界生成页面继续，减少重复等待并保护未完成内容。
- “运行记录”会展示世界骨架生成的状态和恢复位置，具体操作仍在世界生成页面完成。
- 某个阶段遇到思考额度耗尽时，会先只对当前阶段关闭思考后重试，避免整套世界重新生成。
- 模型端点提示请求过大或上下文超限时，世界生成会给出减少参考内容或拆分生成的明确建议；阶段之间只传递必要摘要和稳定 ID，降低长设定反复累积导致中断的概率。
- 通过 OpenCode Go 使用 DeepSeek V4、GLM-5.3 Flash 等模型生成世界骨架时，会按网关能力选择关闭或降低思考档位，减少长时间思考后没有可用结果的情况；普通正文创作仍可保留思考设置。
- 使用 OpenCode Go 或智谱 GLM-5 系列生成结构化世界资料时，会遵循对应端点支持的思考开关和最低推理档位，让阶段输出额度更多用于可保存的设定内容。
- 标准规模世界生成会在地点阶段明确生成地图坐标、方位和行动条件，生成结果更容易直接保存并进入开局展示。
- 开局入口整理会使用明确的地点和具体势力 ID，减少生成内容与世界结构无法对应的情况。
- 世界骨架完整度统一按百分制显示，切换模型后仍能直接比较结果。
- 如果当前阶段的请求超过模型端点可接受的范围，系统会自动压缩可选参考内容并在本阶段重试一次，已完成阶段和已保存数据不会被重复生成。
