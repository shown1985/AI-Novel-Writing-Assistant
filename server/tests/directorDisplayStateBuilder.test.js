...
test("display state maps chapter draft execution into chapter stage and uses fact progress", () => {
  ...
  // Display stages: project_setup, story_planning, world_setup, character_setup,
  // volume_strategy, structured_outline, chapter_execution, quality_repair.
  ...
  assert.equal(displayState.steps[6].key, "chapter_execution");
...
test("display state keeps running mode when task is running despite stale approval projection", () => {
  ...
  assert.equal(displayState.isLiveRunning, true);
  assert.equal(displayState.steps[5].key, "structured_outline");
