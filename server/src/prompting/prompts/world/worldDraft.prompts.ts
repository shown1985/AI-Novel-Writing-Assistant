...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      input.storyEntryBatch
        ? `这是分批整理中的第 ${input.storyEntryBatch.position}/${input.storyEntryBatch.total} 批，只生成本批入口，不得重复前面批次。`
        : "",
    ...
    new HumanMessage([
      ...
      input.storyEntryBatch?.excludedTitles.length
        ? `不得重复的已有入口标题：${input.storyEntryBatch.excludedTitles.join("、")}`
        : "",
