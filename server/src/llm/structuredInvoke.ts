  ...
  try {
    ...
    const collected = await runWithEnforcedTimeout({
      ...
      run: async (signal) => {
        ...
        const trailingReasoning = reasoningCollector.flush();
        reasoningChars += trailingReasoning.length;
        liveSession.reasoning(trailingReasoning);
...
async function tryStructuredStrategies<T>(input: {
  ...
  for (let index = 0; index < preferredSequence.length; index += 1) {
    ...
    try {
      ...
      // Budget exhaustion, truncation and oversized requests cannot be fixed by switching the
      // structured strategy; only failures explicitly marked retryWithNextStrategy (for example an
      // empty json_object response from DeepSeek Flash) continue to the next strategy.
      if ([
        "transport_error",
        "request_too_large",
        ...
        "output_truncated",
        "empty_content",
      ].includes(lastError.category) && !lastError.retryWithNextStrategy) {
        break;
      }
