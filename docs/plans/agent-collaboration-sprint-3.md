...
`S3-03a1` 已在 R1-S3A 完成 `5/5`，四个现有写入口、四类旧记录、版本递增和同步/旧切片阶段例外均有证据，且通过 beta 组合验证。`S3-03a2` 仍保持 `Refinement`，需由后续 Planning 独立评估旧切片主读取切换；S3-03b 仍为独立 5 点卡，不因 a1 完成自动进入实施。
