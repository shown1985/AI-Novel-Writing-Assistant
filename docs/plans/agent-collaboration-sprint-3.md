...
`S3-03a1` 已在 R1-S3A 完成 `5/5`，四个现有写入口、四类旧记录、版本递增和同步/旧切片阶段例外均有证据，且通过 beta 组合验证。`S3-03a2` 的 5 点合同已完成 PO、Scrum Master、QA、QC 复核，并在 R1-S3B 单独承诺：范围只包含内部版本化缓存指纹、仅 slice legacy 的 raw slice 兼容、实例缓存单写、条件写零命中和无世界绝对零写入。S3-03b 仍为独立 5 点卡，不因 a2 进入实施而提前启动。
