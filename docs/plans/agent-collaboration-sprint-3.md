...
AI 结构补全已由 R1-S3D 的 [S3-02b3s 幂等与费用边界 Spike](./s3-02b3s-structure-backfill-idempotency-spike.md) 冻结调用前 claim、生成后 CAS 和响应丢失恢复合同，`2/2` 点完成；这只证明隔离原型可行。生产父项已拆为 a～e 候选，均须分别满足 DoR 和后续 Planning。单区块 `POST /structure/generate` 与其他旧入口不在该 Spike 范围，生产 `/backfill` 仍未获得幂等或版本保护。
