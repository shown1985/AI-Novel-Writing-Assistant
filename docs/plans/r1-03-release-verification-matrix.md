...
| R1-G01 | beta 组合验证覆盖迁移与候选包装；公开发布只由与 `desktop/package.json` 完全匹配的 `vX.Y.Z` tag 触发 | Windows x64 / macOS arm64 | CI/发布治理 | 静态盘点：`node scripts/release/r1-03-static-gate-audit.cjs --strict` | R1-RC02 / 发布 owner | **阻断**：公开 workflow 接受 `desktop-v*` 和 `workflow_dispatch`；且没有 macOS job | 修独立发布治理 Story 后重跑；R1-03 不修改 workflow，不运行 publish |
