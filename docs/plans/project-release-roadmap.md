...
| R1-S2 单书工作台 | 正文优先、一个推荐动作、模型来源可追溯 | `S2-01～04` 的细分卡；前两窗补齐 P0 配置/诊断 Backlog | S2A～S2F 已通过（S2F `8/8`）；R1-S2G 当前承诺 S2-04b4（5 点），只做三入口内部归因/读投影，不接 04c UI/API |
| R1-S3 可信世界 | 世界版本、评估来源、作者决定和历史可信 | `S3-00～06` 的细分卡 | S3-01、S3-02a 已完成；R1-S2G 当前仅承诺 S3-02b1（3 点）的 `updateWorld`/`updateAxioms` CAS 兼容接线，原 S3-02b 其余旧入口保持 Refinement |
...
R1-S2G 是 Release 1 当前实施 Sprint，承诺 `8` 点、无 Stretch，规划基线为 `codex/r1-s2g-attribution-world-writes@32b2e9c7`。两张卡在规划冻结后进入 `In Progress` 并按单一 owner 派发：

| Story | 点数 | 当前范围 | 明确不带入 |
| --- | ---: | --- | --- |
| [S2-04b4 首批身份归因与内部读投影](./s2-04b4-attribution-read-projection-contract.md) | 5 | 自动导演完整 runtime frame、本书 `novel-world-generate`、章节 `ai-revision-preview` 三个显式 context；director frame 整体优先；内部 read service/tests；`reconstructRequest=null` 仅 `not_found`，归因使用 `attributionStatus`，execution `evidenceStatus` 只透传真实 evidence | 公开 API/UI、shared/public DTO、schema/migration、S2-04c、其他入口/batch/旧记录回填 |
| [S3-02b1 世界编辑与公理保存 CAS 兼容](./s3-02b1-world-edit-axiom-cas-contract.md) | 3 | `WorldService.updateWorld` 兼容 HTTP/API；既有 `updateAxioms` UI 显式 revision + 稳定 operationId；统一 CAS；缺保护 428、冲突 409 | 普通编辑新 UI、原 S3-02b 父项其余旧写入口、提案/评估/同步/快照、schema/migration |

两张卡均已写明 DoR、AC、owner、非范围和最窄验证。S2-04c、S3-03～06、原 S3-02b 父项其余旧世界入口、S4+ 均不因本窗口 Planning 自动标为 Ready；本窗口完成不等于 Release 1 可发布。
