...
| R1-S2 单书工作台 | 正文优先、一个推荐动作、模型来源可追溯 | `S2-01～04` 的细分卡；前两窗补齐 P0 配置/诊断 Backlog | [R1-S2A 14 点](./r1-s2a-sprint-commitment.md)、[R1-S2B 9 点](./r1-s2b-sprint-commitment.md)、[R1-S2C 8 点](./r1-s2c-sprint-commitment.md)已通过；[R1-S2D 9 点](./r1-s2d-sprint-commitment.md)实现展示模型、通用 attempt store 与 Prompt execution 边界 |
| R1-S3 可信世界 | 世界版本、评估来源、作者决定和历史可信 | `S3-00～06` 的细分卡 | 所有开启入口均有 revision/历史保护 |
