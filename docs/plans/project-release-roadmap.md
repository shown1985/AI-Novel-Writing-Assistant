...
| R1-S1 配置与安全阅读 | 被动查看不产生模型调用，阅读可恢复，世界写入先校验归属 | `S1-00～06` | S1-A/S1-B 分窗通过 |
| R1-S2 单书工作台 | 正文优先、一个推荐动作、模型来源可追溯 | `S2-01～04` 的细分卡 | S2-A/S2-B 分窗通过 |
| R1-S3 可信世界 | 世界版本、评估来源、作者决定和历史可信 | `S3-01～06` 的细分卡 | 所有开启入口均有 revision/历史保护 |
...
- 状态/Owner/依赖：Ready；根集成人；已同步最新`main`。
...
- 状态/Owner/依赖：Ready；Runtime审计owner；R1-00。
...
- 状态/Owner/依赖：Ready；验收owner；R1-00与既有主链测试inventory。
...
- 状态/Owner/依赖：待解锁；PO/根集成人；R1-00～03。
