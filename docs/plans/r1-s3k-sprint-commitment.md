...
- GPT-6 Luna Medium QA/QC 独立核查：真实 workflow guard 对 `v0.4.28`、`v1.2.3`（fixture）、`desktop-v*`、`-rc1` 的输出；发布脚本（复制进临时仓库、配合本地 bare remote）按 fetch/push URL 判定上游并拒绝推送目标为上游、本地/目标/上游已有 tag、major 0，且在无上游 remote 或 `ls-remote` 失败时明确拒绝；G01a 第 74-92 行正例已迁到临时目录 fixture；四处 owner、上游 owner 精确路径加命中数白名单（含 trigger 脚本第二处命中）、beta 禁用串、跨 workflow 写权限/`write-all`/缺省顶层权限/四种令牌引用的突变断言，以及 `site-pages.yml` 正例；Windows 发布 job 仍是唯一写权限 job；文件边界。Scrum Master 审查 WIP、DoR/DoD、范围与 Sprint Review。
...
- 出现以下任一情况时停回 Refinement 由 PO 拆分，不扩大范围：需要改 `desktop/src` 或 `server/src`、需要新增 workflow、需要真实网络或 Actions 才能验证，或工作量超过 7 点。
