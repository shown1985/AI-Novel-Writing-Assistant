...
| --- | ---: | --- | --- | --- | --- |
| S1-01 缺省数值配置正确生效 | 3 | Ready（Wave 2） | 配置 Agent | 无 | 未配置、空白和坏值使用声明默认，合法低值与允许的 0 保持 |
...
| S2-02 专业章节辅助区域按需展开 | 3 | In Progress | 章节编辑 Agent；根集成人负责外部身份接线 | S1-X 未实施，现有修订链可复用 | 正文默认优先，目录和 AI 协作按需展开，折叠与同章刷新不丢草稿或候选 |
