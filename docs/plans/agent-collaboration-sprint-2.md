...
### R1-PROMPT02：结构化请求级物理调用预算

- 用户价值：显式要求“最多一次供应商调用”的操作，不会因日后新增的重试或修复路径悄悄多花一次调用。
- 状态：**Refinement / Not Ready**；未估点，未承诺任何 Sprint。来源：R1-S3I 独立 QA/QC 对 `S3-02b3b2a` 的低级发现。
- 已知事实：b3b2a 的单次模式靠逐个关闭传输重试、策略切换、备用模型、JSON 修复和语义重试实现，每个守卫均有变异测试覆盖；但没有统一计数器，新增的重试路径不会被自动限额。
