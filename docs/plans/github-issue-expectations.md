...
- 优先级：P1
- 实施状态：已合入 `main`（`a580a27`），随桌面 0.4.18 发布（提交 `3ee10ac`），未经用户界面验收
- 关联 PR：无（未经过个人仓库 PR 流程，改动直接合入 `main`）
- 实施分支：`codex/issue-126-model-controls`（已合入，可清理）
- 对外动作：功能已发布，是否回复 Issue 由用户决定；无需再推送分支或创建 PR
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 PR：个人仓库 PR #1（阶段 1），内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P1
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 PR：个人仓库 PR #2（阶段 2，基于阶段 1 分支），内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P1
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 PR：[个人仓库 PR #3](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/3)（阶段 3，基于阶段 2 分支），内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 PR：[个人仓库 PR #4](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/4)（阶段 4，基于 WGR-004 分支），内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 PR：[个人仓库 PR #5](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/5)（阶段 5，基于 WGR-005 分支），内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P1
- 状态：已合入 `main`（`a580a27`），未经用户验收；`main` 上的集成回归见 `TASK.md` P2-1
- 关联 PR：[个人仓库 PR #6](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/6)，内容已随 `a580a27` 进入 `main`，PR 已关闭
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），未经用户真实模型验收（云端环境无模型 API Key，需用户本机验证）
- 关联 PR：无（未单独开 PR；改动已随对应提交合入 `main`）
- 实施分支：`fix/world-generation-budget-preflight`（已合入，可清理）
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），未经用户真实模型验收
- 关联 Issue：个人仓库 Issues 当前关闭（GitHub API 返回 410），以下编号继续作为本地索引
- 实施分支：`codex/world-generation-opencode-capability`（已合入，可清理）
...
- 当前证据：OpenCode Go 端点识别、DeepSeek V4/GLM-5.3 Flash 结构化关闭思考、普通文本保留思考、官方与未知自定义端点回归测试均已覆盖；服务端构建、根 typecheck，以及 LLM/推理/会话/结构化调用/世界骨架定向测试共 49 项全部通过。GitHub Issues 当前关闭，远端 Issue 无法创建（API 410）；个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#8](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/8)，已关闭（内容已随 `a580a27` 进入 `main`）。真实 OpenCode Go 模型和 UI 世界生成仍需用户本机验收。
...
- 状态：已合入 `main`（`a580a27`），未经用户真实模型验收
- 关联 Issue：个人仓库 Issues 当前关闭（GitHub API 返回 410），以下编号继续作为本地索引
- 实施分支：`codex/opencode-glm-thinking-toggle`（已合入，可清理）
...
- 当前证据：官方 GLM 与 OpenCode Go 分支参数映射、DeepSeek 保持思考开关、普通文本行为的回归测试已通过；服务端构建与根 typecheck 通过，LLM/推理/会话/结构化调用/世界骨架定向测试共 49 项全部通过。真实 OpenCode Go 回放确认 GLM-5.3 Flash 拒绝 `thinking.type=disabled`（返回错误码 1210，要求使用 low/high/max），该端点契约已按官方网关行为调整；完整标准规模回放还被地点阶段提示契约缺陷阻断，转入 WGR-011 修复。提交：`1b6c5ba1`；个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#9](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/9)，已关闭（内容已随 `a580a27` 进入 `main`）。
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：个人仓库 Issues 当前关闭（GitHub API 返回 410），以下编号继续作为本地索引
- 实施分支：`codex/world-location-prompt-contract`（已合入，可清理）
...
- 当前证据：地点阶段此前因提示示例缺少地图字段而在装配校验失败；补齐示例后，真实回放耗时约 54.6 秒并完整通过，无 `too big`、无错误码 1210。服务端构建与地点/世界骨架定向测试 8 项全部通过。提交：`85a48454`；个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#10](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/10)，已关闭（内容已随 `a580a27` 进入 `main`）。
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：个人仓库 Issues 当前关闭（GitHub API 返回 410），以下编号继续作为本地索引
- 实施分支：`codex/world-presentation-context-contract`（已合入，可清理）
...
- 当前证据：小型展示探针从约 1,600 字符输入在 4.9 秒内完成；压缩上下文后 DeepSeek 入口 ID 仍暴露 faction 引用，补充具体 ID 清单并移除 factions 后，完整回放成功。DeepSeek standard 回放耗时约 64.5 秒，规则 5、阵营 3、势力 5、地点 6、势力关系 6、地点控制 6、故事入口 3，`readyForNovelUse=true`；GLM standard 回放耗时约 54.6 秒同样通过。提交：`55dfd7e8`；个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#11](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/11)，已关闭（内容已随 `a580a27` 进入 `main`）。
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：个人仓库 Issues 当前关闭（GitHub API 返回 410），以下编号继续作为本地索引
- 实施分支：`codex/world-assessment-score-contract`（已合入，可清理）
...
- 当前证据：服务端构建与根类型检查通过，相关回归测试 52/52 通过；DeepSeek 回放返回 `85`，耗时约 43.2 秒；GLM 回放返回 `92`，耗时约 79.0 秒，均为整数且 `readyForNovelUse=true`。提交：`7ba8fcf6`；个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#12](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/12)，已关闭（内容已随 `a580a27` 进入 `main`）。
...
- 实施分支：`codex/world-generation-budget-runtime`（已合入，可清理）
...
- 个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#14](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/14)，基于 `codex/macos-wgr-acceptance`，已关闭（内容已随 `a580a27` 进入 `main`）。
...
- 优先级：P1
- 状态：已合入 `main`（`a580a27`），自动验收脚本已跑通，仍待用户在真机上验收首次配置、世界生成和 Dock 再激活
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 实施分支：`codex/macos-runtime-acceptance`（已合入，可清理）
...
- 个人仓库 PR：[shown1985/AI-Novel-Writing-Assistant#15](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/15)，基于 WGR-014/015 PR #14，已关闭（内容已随 `a580a27` 进入 `main`）；真实模型、首次配置和 Dock 再激活仍待用户本机验收。

### WGR-017：Agent Runtime 控制平面 MVP

- 优先级：P1
- 状态：已合入 `main`（`a580a27`）；架构审查确认沿用现有 Runtime，未新增代码，未经用户验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #16](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/16)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/agent-runtime-control-plane`（已合入，可清理）
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #17](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/17)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/world-generation-budget-observability`（已合入，可清理）
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #18](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/18)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/world-generation-opencode-session`（已合入，可清理）
...
- 当前证据：`pnpm --filter @ai-novel/server build` 通过；世界骨架、OpenCode 会话和 LLM provider 定向测试 23 项通过、0 项失败。个人仓库 PR #18 基于 WGR-018 分支，已关闭（内容已随 `a580a27` 进入 `main`）；真实模型和 Mac 人工验收仍待用户本机完成。
...
- 状态：已合入 `main`（`a580a27`），未经用户验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #19](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/19)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/opencode-header-transport-test`（已合入，可清理）
...
- 当前证据：`pnpm --filter @ai-novel/server build` 通过；LLM provider、OpenCode 会话和世界骨架定向测试 24 项通过、0 项失败；当前分支重新构建的 `0.4.18` Mac arm64 包已通过布局检查及 DMG 独立运行/重启持久化验证。个人仓库 PR #19 基于 WGR-019 分支，已关闭（内容已随 `a580a27` 进入 `main`）；真实模型、首次配置、世界生成和 Dock 再激活仍待用户本机人工验收。

### WGR-021：Mac 打包后的宿主原生依赖隔离

- 优先级：P1
- 状态：已合入 `main`（`a580a27`），自动验证已完成，待用户人工验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #20](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/20)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/desktop-native-isolation`（已合入，可清理）
...
- 优先级：P0
- 状态：已合入 `main`（`a580a27`），真实模型验证通过，待用户界面验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[个人仓库 PR #21](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/21)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/llm-selection-hydration`（已合入，可清理）
...
- 当前证据：手工回放显示密钥记录仍存在且 OpenCode Go 已配置，但失败任务种子被写成 `ollama + glm-5.3-flash`，最终在 `127.0.0.1:11434` 产生 `Connection error`；修复加入请求刷新等待与水合保护，定向测试 6/6、客户端类型检查和根 `typecheck` 通过，重新构建的 Mac arm64 `.app` 已启动并保持 OpenCode Go 选择。随后真实中等规模世界骨架六阶段调用全部使用 `custom_opencode_go / glm-5.3-flash` 并返回 200，自动导演入口携带旧 Ollama 字段时也成功使用 OpenCode Go，未出现 `too big`、413 或空响应。

### WGR-023：新建任务使用服务端当前模型选择

- 优先级：P0
- 状态：已合入 `main`（`a580a27`），验证通过，待用户验收
- 关联 Issue：创建失败，个人仓库 API 返回 410
- 关联 PR：[shown1985/AI-Novel-Writing-Assistant#22](https://github.com/shown1985/AI-Novel-Writing-Assistant/pull/22)，内容已随 `a580a27` 进入 `main`，PR 已关闭
- 实施分支：`codex/server-authoritative-task-selection`（已合入，可清理）
...
- 当前证据：真实自动导演请求携带旧 Ollama 字段后，Worker 日志显示资源推荐和候选生成均使用 `custom_opencode_go / glm-5.3-flash`，候选任务成功进入待确认状态；真实中等规模世界骨架的 profile、rules、factions、locations、relations、presentation 六次阶段调用全部返回成功，接口返回 200，未出现 `too big`、413 或空响应。
