...
- GPT-6 Luna Medium QA/QC 独立核查：provider `stream()` 层实际调用次数、调用前 `model_in_flight` 持久事实、双连接并发、lease/unknown 不重调、`malformed_json`/`empty_content` 分类、零 World 写入与文件边界。Scrum Master 审查 WIP、DoR/DoD、范围与 Sprint Review。
