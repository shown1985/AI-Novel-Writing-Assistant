...
当前规则：该门是 `PromptExecutionOptions.singleProviderTransportAttempt`，只对非流式 `runStructuredPrompt` 生效，一次逻辑请求内传输重试、策略切换、备用模型、JSON 修复与语义重试合计最多一次底层 provider 调用；失败原样交给调用方决定，运行器不重调。`streamStructuredPrompt` 收到该选项时在打开 provider 前拒绝；`runTextPrompt`/`streamTextPrompt` 会忽略该选项，需要单次保证的调用方不得经文本入口。该门逐项关闭各重试路径而非统一计数，新增重试或修复路径时必须同步让单次模式跳过并补 transport 计数测试。
