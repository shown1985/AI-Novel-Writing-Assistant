...
- 用户未选择参考世界样本时，默认根据宏观规划与书级约定自动生成本书 `NovelWorld`。当前创建服务同时生成关联世界样本并绑定回本书；兼容参数 `saveToLibrary=false` 不关闭该行为。本书世界与来源样本仍是独立副本，后续同步须明确确认。
...
- 自动导演只依赖 `WorldContextGateway`，不直接调用旧的小说世界生成入口。首次创建的样本关联由本书世界创建服务承接；章节演进和本书修改不得自动回写来源样本。
...
- 如果把首次创建关联样本误解为自动持续同步，章节演进或本书修改会污染可复用样本。来源副本的创建与后续 `push/pull` 必须区分，样本不得自动覆盖已导入的本书世界。
