...
- 简易书架会按作品和章节保存阅读现场；刷新、重新进入或使用浏览器前进后退时可以继续阅读同一章，并在正文变化后按阅读进度回到合适位置。存储不可用时仍可正常阅读，切换作品不会沿用上一部作品的章节或位置。
- 专业章节编辑器以正文为中心，章节参考与 AI 协作可按需展开；宽屏可以同时使用两个辅助区域，窄屏一次只显示一个，折叠和重新打开不会丢失草稿、选区、修订指令或 AI 候选。
- 章节 AI 诊断只在作者首次打开辅助区域或明确发起修订时运行；仅进入页面、折叠重开或选择正文不会重复调用。检测到外部正文变化时会先请作者选择保留草稿或载入新正文，避免静默覆盖编辑现场。

#### 修复

- 切换作品后，上一部作品迟到的继续、恢复、审批、重试或取消结果不会再改写当前作品的页面状态、地址或提示；质量优先流程的人工恢复暂停也会在后台轮询时保持。
- 修复 AI 资料检索与写法提取的数值配置在未填写、留空或包含坏值时可能误用极小值的问题；系统会采用声明的可靠默认值，同时保留作者明确设置的合法低值与允许的零值，读取设置不会触发索引任务或改写配置。
- 单机成书版的网页开发服务和业务 API 只接受本机访问；局域网、通配地址或远程网页来源配置会在读取或修改作品前被拒绝，避免无账号环境暴露小说内容和模型密钥。
- 修复视觉资产两条升级历史合流后可能重复创建漫画角色字段、场景和角色资产表的问题；新安装、旧宽迁移历史、细分修复历史和中断后重启都会沿同一安全升级路径保留已有作品数据。
- 修复在一个世界中处理一致性问题时可能先改写另一个世界记录的问题；错归属和不存在的请求会保持原记录不变并返回明确错误。
...
- 中等或复杂世界生成遇到地点输出过长、为空或字段不完整时，会自动拆成更小批次继续；每批成功内容都会保留，应用中断后无需重做已经完成的世界结构。
- 最后的开局整理若无法一次返回全部故事入口，会先压缩内容并关闭思考重试，仍不完整时逐个生成入口并自动合并。
- 世界生成在后台完成后，重新打开生成页会直接回到骨架预览；正式保存前即使重启应用也能继续查看，不会清空结果或让用户重新填写。
- 视觉资源库会兼容旧版桌面数据库，并隔离图片、漫画和短剧素材来源；单个来源暂时不可用时仍可浏览其他素材，加载失败会在资源库内提供重试，不再连续弹出多条错误提示。
- 写法引擎会把“我的写法”和随应用提供的“官方起步写法”清楚分开；左侧选择资产，右侧按概览、剧情推进、人物表达、语言质感、节奏控制和反 AI 约束阅读，不再把长文字挤进多列卡片。应用测试成为主要操作，删除收进详情底部，降低误操作压力。
- 在世界样本手册中编辑概要、规则、势力、地点或张力时，会进入专注的编辑视图；可以“保存并返回”更新手册，或“取消返回”放弃本次改动，不必再在概览与编辑框之间来回滚动。

### 2026-09-09（导演跟进筛选显示）

- 导演跟进中的“全部原因”“全部状态”和“批量能力”等筛选项会保持单行显示；窗口变窄时会在控件内部收缩，不再把最后一个字挤到下一行。

### 2026-09-06（世界观向导连接稳定性）

- 使用 OpenCode Go 创建世界观时，同一次生成的各阶段请求会被识别为同一个会话，并保留当前阶段信息，减少网关因缺少会话标识而拒绝请求的情况。
- 生成中断后从检查点继续时会沿用原会话识别，便于服务端区分阶段重试并继续优化请求处理。
- 快速配置完成后会稳定使用你刚选择的模型厂商，不会因配置刷新短暂回退到本地服务，减少重复输入 API Key 和误报连接失败。
- 创建自动导演任务时会沿用已选的模型厂商和模型，避免旧页面状态把新的世界观请求误发到本地服务。

### 2026-09-05（世界骨架生成稳定性）

- 使用 GLM-5.3 Flash 等推理模型生成世界骨架时，会优先采用较低的思考深度，让更多输出额度用于完整的世界设定。
- 世界骨架没有生成正文时，向导会区分思考额度耗尽、输出达到上限和服务无响应，并给出降低思考深度、缩小规模、重试或切换模型的下一步建议。
- 生成失败不会保存不完整的世界数据；JSON 修复也会沿用本次生成的思考深度设置。
- 标准长篇和复杂群像会把世界骨架拆成多个较小阶段完成，再统一检查数量、引用和地图信息，减少一次输出过长导致的中断；轻量舞台仍保持快速生成。
- 标准长篇和复杂群像生成会保存阶段检查点；网络中断或模型输出失败后，重新打开世界生成页即可从最近完成的阶段继续，不会重复生成前置内容，也不会把半成品保存为世界。
- 世界骨架的状态、失败阶段和来源位置会同步出现在“运行记录”中；恢复仍需回到世界生成页面完成。
- 如果某个世界骨架阶段明确因为思考额度耗尽而失败，系统会只对当前阶段关闭思考后再试一次，减少整套世界重复生成和等待。
- 模型端点返回“请求过大”或上下文超限时，世界生成会直接说明需要减少参考内容或拆分生成，不再把这类服务端边界误显示为普通 JSON 错误。
- 世界骨架的后续阶段只携带当前需要的短摘要和稳定实体 ID，完整设定仍会保存并用于页面展示；长描述不会随着阶段推进反复扩大请求。
...
- 标准规模世界生成会在地点阶段明确生成可绘制地图坐标、方位和进入/离开条件，减少模型已经生成地点却无法保存的情况。
- 世界骨架最后的开局整理会只使用必要的势力和地点摘要，并明确可引用的具体 ID，减少最后阶段等待过久或入口无法保存的情况。
- 世界骨架完整度会统一按百分制展示，不同模型不会再因为评分量纲不同而显示异常数值。
- 当模型端点明确返回请求过大或上下文超限时，当前世界生成阶段会自动缩短可选的长描述并降低本阶段输出额度后重试一次；已完成的前置阶段不会重复生成，仍无法完成时会保留清晰的缩小规模或拆分建议。
...
### 2026-09-03（Apple Silicon Mac 桌面版）

- Apple Silicon Mac 可以在本机生成并运行桌面客户端，提供应用程序、DMG 安装盘和 ZIP 包；Windows 安装版继续使用原有构建方式。
- Mac 桌面版会携带本地写作服务、页面资源、数据库能力和图片处理能力，安装包会在生成后检查关键组件是否属于 arm64，减少“能安装但启动失败”的情况。
- 关闭 Mac 客户端的最后一个窗口后，可以从 Dock 再次打开工作区；本地写作服务会继续复用，不会因窗口恢复而重复启动。
- 本地测试包不会连接公开更新通道，避免尚未发布 Mac 版本时显示无效的更新失败；后续完成 Apple 签名与公证后再启用公开更新。
